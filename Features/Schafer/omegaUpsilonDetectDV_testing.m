function [omegaFrames, upsilonFrames] = omegaUpsilonDetectDV_testing(x, y, lenArray, frameClass)
...
tmpArray = angleArray;
if sum(~isnan(tmpArray)) > 2
    tmpArray(isnan(tmpArray)) = interp1(...
        find(~isnan(tmpArray)),...
        tmpArray(~isnan(tmpArray)),...
        find(isnan(tmpArray)),'nearest');
    % Step 2: get the diff and identify the jump frames
    tailToHeadDirectionInterpDiff = diff(tmpArray);
    positiveJumps = find(tailToHeadDirectionInterpDiff > pi) + 1;
    negativeJumps = find(tailToHeadDirectionInterpDiff < -pi) + 1;
    
    % subtract pi from remainging data after positive jumps
    for j = 1:length(positiveJumps)
        angleArray(positiveJumps(j):end) = angleArray(positiveJumps(j):end) - 2*pi;
    end
    
    % add pi to remaining data after negative jumps
    for j = 1:length(negativeJumps)
        angleArray(negativeJumps(j):end) = angleArray(negativeJumps(j):end) + 2*pi;
    end
end
