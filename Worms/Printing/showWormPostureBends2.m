function [saveFile pages] = showWormPostureBends2(worm, varargin)
%SHOWWORMPOSTUREBENDS2 Show the worm bends standard deviation posture.
%
%   [SAVEFILE PAGES] = SHOWWORMPOSTUREBENDS2(WORM)
%
%   [SAVEFILE PAGES] = SHOWWORMPOSTUREBENDS2(WORM, FILEPREFIX)
%
%   [SAVEFILE PAGES] = SHOWWORMPOSTUREBENDS2(WORM, FILEPREFIX, PAGE)
%
%   [SAVEFILE PAGES] = SHOWWORMPOSTUREBENDS2(WORM, FILEPREFIX, PAGE,
%                                            ISCLOSE)
%
...
%% Show the standard deviation of bends.
...
% Show the worm standard deviation of bends.
subplot(2, 3, 1);
hold on;
segNames = {
    'head', ...
...
for i = 1:length(segNames)
    data{i} = worm.posture.bends.(segNames{i}).stdDev;
    dataNames{i} = [upper(segNames{i}(1)) segNames{i}(2:end) ' S.D. of Bends'];
end
samples = sum(~isnan(data{1}));
titleName = ['WORM S.D. of BENDS (N = ' num2str(samples) ')' titlePad];
xAxisName = 'Standard Deviation of Bends (degrees)';
resolutions = 0.5;
...
% Show the forward/paused/backward worm standard deviation of bends.
dataAll = data;
dataAllNames = dataNames;
dataNames = cell(length(motionModes), 1);
for i = 1:length(motionModes)
    dataNames{i} = [motionNames{i} ' S.D. of Bends'];
end
locations = {
    2, ...
    5, ...
...
if ~isempty(varargin) && ~isempty(varargin{1})
    ...
    % Construct the title.
    figureTitle = 'POSTURE: STANDARD DEVIATION OF BENDS';
    ...
    saveFile = [varargin{1} '_posture_bends2.pdf'];
