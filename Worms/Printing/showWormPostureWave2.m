function [saveFile pages] = showWormPostureWave2(worm, varargin)
%SHOWWORMPOSTUREWAVE2 Show the worm amplitude-ratio, eccentricity, and
...
% Show the worm amplitude ratio.
subplot(2, 3, 1);
hold on;
data = {worm.posture.amplitude.ratio};
titleName = ['WORM AMPLITUDE RATIO' titlePad];
xAxisName = 'Amplitude Ratio Min/Max (unitless measure)';
dataNames = {'Amplitude Ratio'};
resolutions = 0.01;
...
% Show the forward/paused/backward worm amplitude ratio.
subplot(2, 3, 4);
hold on;
dataAll = worm.posture.amplitude.ratio;
...
titleName = ['WORM AMPLITUDE RATIO (N = ' num2str(samplesAll) ...
    ', MEAN = ' num2str(meanAll) ...
    ', S.D. = ' num2str(stdDevAll) ')' titlePad];
xAxisName = 'Amplitude Ratio Min/Max (unitless measure)';
dataNames = cell(length(motionModes), 1);
for i = 1:length(motionModes)
    dataNames{i} = [motionNames{i} ' Amplitude Ratio'];
end
resolutions = 0.01;
...
% Show the worm eccentricity.
subplot(2, 3, 2);
hold on;
eccentricity = 1./ log(1 ./ (1 - worm.posture.eccentricity));
data = {eccentricity};
titleName = ['WORM ECCENTRICITY' titlePad];
xAxisName = 'Eccentricity (unitless measure)';
...
% Show the forward/paused/backward worm eccentricity.
subplot(2, 3, 5);
hold on;
dataAll = eccentricity;
data = cell(length(motionModes), 1);
for i = 1:length(motionModes)
    data{i} = dataAll(motionMode == motionModes{i});
end
...
meanAll = nanmean(dataAll);
stdDevAll = nanstd(dataAll);
titleName = ['WORM ECCENTRICITY (N = ' num2str(samplesAll) ...
    ', MEAN = ' num2str(meanAll) ...
    ', S.D. = ' num2str(stdDevAll) ')' titlePad];
xAxisName = 'Eccentricity (unitless measure)';
dataNames = cell(length(motionModes), 1);
for i = 1:length(motionModes)
    dataNames{i} = [motionNames{i} ' Eccentricity'];
end
resolutions = 0.1;
histColors = str2colors('rkb', 0, true);
statColors = str2colors('rkb', -0.1, true);
...
%% Show the track length.

% Show the worm eccentricity.
subplot(2, 3, 3);
hold on;
data = {worm.posture.tracklength};
titleName = ['WORM TRACK LENGTH' titlePad];
xAxisName = 'Track Length (microns)';
dataNames = {'Track Length'};
resolutions = 5;
histColors = str2colors('k', 0, true);
statColors = str2colors('r', -0.1, true);
plotHistogram(data, resolutions, titleName, xAxisName, dataNames, ...
...
subplot(2, 3, 6);
hold on;
dataAll = worm.posture.tracklength;
data = cell(length(motionModes), 1);
for i = 1:length(motionModes)
    data{i} = dataAll(motionMode == motionModes{i});
end
...
meanAll = nanmean(dataAll);
stdDevAll = nanstd(dataAll);
titleName = ['WORM TRACK LENGTH (N = ' num2str(samplesAll) ...
    ', MEAN = ' num2str(meanAll) ...
    ', S.D. = ' num2str(stdDevAll) ')' titlePad];
xAxisName = 'Track Length (microns)';
dataNames = cell(length(motionModes), 1);
for i = 1:length(motionModes)
    dataNames{i} = [motionNames{i} ' Track Length'];
end
resolutions = 5;
histColors = str2colors('rkb', 0, true);
statColors = str2colors('rkb', -0.1, true);
...
if ~isempty(varargin) && ~isempty(varargin{1})
    % Construct the title.
    figureTitle = 'POSTURE: AMPLITUDE RATIO, ECCENTRICITY, AND TRACK LENGTH';
    ...
    % Save the figure.
    saveFile = [varargin{1} '_posture_wave2.pdf'];
