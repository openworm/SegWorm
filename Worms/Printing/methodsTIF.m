...
'resolution, 30fps, with a magnification that results in 3.5-4.5 microns/pixel'
...
'to avoid exciting a short-wavelength avoidance response through the \itC.\rm'
'\itelegans\rm LITE-1 receptor. Finally, the motorized stage uses Zaber T-NA0850'
'linear actuators. The stage travels 5cm at up to 8mm/s in orthogonal x and y'
'axes with a resolution of just under 0.05\mum.'
''
...
'(Brenner 1974) seeded with 3 drops of OP50. 6 adult worms are transferred to a'
'new plate to maintain stocks. At least two generations are passaged, in these'
'conditions, prior to tracking. The evening before tracking, at roughly 5pm, L4'
...
'on one of the eight nearby trackers, serving as a control. Strains are tracked'
'between 8am and 6pm, across several hours and days, and randomly assigned to any'
'of eight available trackers. In total (prior to filtering the videos for'
'quality) approximately 25 worms are tracked, per experimental strain, controlled'
'by roughly 65 N2s tracked within one week of the experiments.'
};
end
...
'dorsal-ventral orientation. When the worm has its ventral side concave within'
'the bend, the bending angle is signed negatively.'
''
...
'individual videos and 95.6% of the video frames collectively.'
''
...
'the two possible head-tail orientations minimizes the distance between'
...
'for feature computation. Wild-type worms have four quadrants of longitudinal,'
...
'midpoint. Moreover, since the worm is confined to two dimensions, its bodywall'
...
'of freedom and, therefore, expect to be sampling above the Nyquist rate for worm'
'posture.'
''
'A common notation is used to define the body parts. The head is controlled by'
'the first four bodywall muscles, per quadrant -- approximately 1/6 the length of'
'the worm (White et al. 1986). Similarly, the neck is controlled by the next four'
...
'maximum amplitude is defined as the maximum y coordinate minus the minimum y'
...
'events. First, if we find fewer than two sharp ends on the contour (reflecting'
'the head and tail) then the head and/or tail are obscured in a coiling event.'
...
'midbody, to its tail. The worm''s body is separated into three equal parts from'
'its head to its tail. The mean supplementary angle is measured along each third.'
'For omega turns, this angle must initially exceed 30\circ at the first but not'
'the last third of the body (the head but not the tail). The middle third must'
'then exceed 30\circ. And finally, the last but not the first third of the body'
'must exceed 30\circ (the tail but not the head). This sequence of a 30\circ mean'
...
'third of the body) while not exceeding 30\circ on the opposite end.'
};
end
...
'for the worm''s instantaneous path curvature. We use the difference in tangent'
'angles between the middle to the end and between the start to the middle. The'
'distance is measured as the integral of the distance traveled, per frame,'
'between the start and end frames. When a frame is missing, the distance is'
'interpolated using the next available segmented frame. The instantaneous path'
'curvature is then computed as the angle divided by the distance. This path'
'curvature is signed negatively if the angle curves in the direction of the'
'worm''s ventral side.'
};
end
...
'has a prefix indicating whether all significant measurements agree that the'
'feature is greater (+), less (-), or different (\Delta) than the control. A'
'feature is said to be different than its control whenever the magnitude has no'
'direct meaning (e.g., asymmetry does not translate to a clear description of the'
'measurement being less nor greater than the control) or its measures do not'
'express a simple magnitude (e.g., the strain pauses with greater frequency but'
'spends less time in each paused event). Each term also has a suffix indicating'
'the minimum q-value (significance) found for the term''s defining measures'
'(* when q \leq 0.05; ** when q \leq 0.01; *** when q \leq 0.001; and,'
'**** when q \leq 0.0001). The q-value is a p-value replacement that corrects for'
'multiple testing (Storey 2002). The ontology terms are as follows:'
''
...
'spreadsheet of the data, per strain. And, three types of MAT files are provided'
'to access the strain data and statistics as well as the skeleton, contour, and'
...
'24 worms per strain with a minimum of 12 and a standard deviation of 14.'
'Controls were chosen from the filtered N2 data collection by matching the strain'
'collections to controls performed within the same week. This resulted in a mean'
'of 63 controls, per strain collection, with a minimum of 18 and a standard'
'deviation of 29. We examined 100 videos (roughly 2 million frames) from our'
'filtered collection and found that the head was correctly labeled with a mean'
'and standard deviation of 95.17 \pm 17.5% across individual videos and 95.69% of'
'the frames collectively.'
''
...
'sets.'
''
'Shapiro-Wilk testing (performed using the "swtest" function by Ahmed Ben SaÃ¯da)'
'of each feature measure (with corrections for multiple comparisons) showed a'
'maximum q-value of 0.0095 over our collective N2 data set, indicating that, in'
'aggregate, none of the measures are normally distributed. Further testing across'
'all strain collections (which have far lower sampling than the N2 collective)'
'and their controls, indicated a roughly 2:1 ratio of normal to non-normal'
'distributions, rejecting the null hypothesis of normality at a q-value of 0.05.'
'Therefore, we chose to test strain measurements against their controls by using'
'the non-parametric Wilcoxon rank-sum test (with the null hypothesis that both'
'sets of mean values were drawn from the same distribution). In four strains, at'
'least one measure was detected exclusively in either the strain or its control,'
'meaning the measurement was always observed within one set and never in the'
'other (e.g., some strains never perform reversals). When this occurred, we used'
'a Fisher''s exact test to measure the probability that our sets were drawn from'
'the same distribution of observed and unobserved events. Occasionally, features'
'measurements had insufficient values for testing due to low sampling (e.g.,'
'omega-turn events), these measures were ignored and their p-value marked as'
'undefined. In total, our 702 measurements were obtained for each of 305 strains'
'in addition to collections of our N2 worms by hour (9am-4pm, with 8am and 5pm'
'discarded due to very low sampling), weekday (Tuesday-Friday, with Monday and'
'Saturday discarded due to very low sampling), and month (January-December). We'
'used False-Discovery Rate (FDR) to correct for nearly 702 measures by 329 groups'
'and transform the p-values to their q-value equivalents (Storey 2002).'
''
...
'measures are shown alongside their minimum q-value and a page number for'
...
'Ontology").'
''
'2. There are three summary pages. These pages show important feature histograms,'
...
'as well; d) the p-value results using Wilcoxon rank-sum testing and q-value'
'results using False Discovery Rate correction (for multiple tests across 329'
'strain collections by 702 feature measurements), both labeled accordingly'
'(respectively "p" and "q"); e) event features also display their mean frequency'
'("FREQ"), the mean percentage of time spent in the event relative to the total'
'experiment time ("TIME"), and, when available, the mean percentage of distance'
'traveled during the event relative to the total distance covered during the'
'experiment ("DIST").'
''
...
'experimental collection is accompanied by four CSV files presenting the data and'
...
'presented for each feature measure, per worm and for the collection of'
'experiments and controls. The p and q-values are presented for the strain as a'
'whole (the null hypothesis is that experiment and control worms are drawn from'
'the same distribution) and for each feature measure individually. These p and q'
'values are shown for both the non-parametric Wilcoxon rank-sum test and the'
'normal-distribution Student''s t-test (unpaired samples with unequal variance).'
'The Shapiro-Wilk test for normality (with associated p and q values) is also'
'shown for each measure. Correction for multiple testing (the q-values) was'
'performed over our entire set of 329 groups of strain collections by 702'
'measures. For the Shapiro-Wilk normality test, correction for multiple'
'comparisons included an additional 329 group-specific controls by 702 measures.'
};
end
...
'share a similar format. The individual experiment files present the feature data'
'as a time series. They also include the full skeleton and the centroid of the'
...
'file types is two top-level structs, "info" ("wormInfo" for the strain'
...
''
'2. video. The video information. The video "length" is presented as both'
'"frames" and "time". The video "resolution" is in "fps" (frames/seconds), pixel'
'"height" and "width", the ratio of "micronsPerPixel", and the codec''s "fourcc"'
...
'"annotations".'
''
'4. files. The name and location for the analyzed files. Each experiment is'
...
'various body parts, and its six "eigenProjections". Individual experiment files'
...
''
'Finally, the strain collection files present their data in three types of'
'fields: a) individually as the "data" per experiment, b) summarized over the'
'"sets" of experiments and, c) aggregated in "allData" as if we ran one giant'
'experiment instead of our sets. In other words, "sets" weights each experiment'
'identically whereas "allData" weights every frame, across all experiments,'
'identically. The data is always represented as both a "mean" and "stdDev"'
'(standard deviation). The mean and standard deviation are always computed for'
'"all" the data. When the data is signed, the mean and standard deviation are'
'also computed for the data''s "abs" (absolute value), "pos" (only the positive'
'values), and "neg" (only the negative values). The format for the three types of'
'data is as follows:'
''
...
'file. This file contains three top-level structs with information for both the'
...
'strains and columns of feature measures. The "worm" struct has the following'
'subfields:'
''
'1. info. The worm information for each strain collection presented as their'
'"strain", "genotype", "gene", and "allele".'
''
'2. stats. The statistics for each strain collection presented, for every feature'
'measure, as their "mean", "stdDev" (standard deviation), "samples" (the number'
'of worms providing a measurement for the feature - e.g., not all worms execute'
'omega turns), and "zScore" relative to the control (a simple normalization to'
'the control -- note that the collection of N2 controls has no zScore).'
...
'Furthermore, we include Shapiro-Wilk tests of data normality, per measure, in'
'"pNormal" and correction for multiple testing, using their False-Discovery rate'
'q-value replacements, in "qNormal". The q-values are computed across all'
'measures per "strain" and their associated controls (roughly 1404 tests) and'
'across "all" strain and control measures collectively (roughly 329 by 1404'
'tests).'
''
'3. sig. The statistical significance for each strain collection is presented,'
'for every feature measure, as their "pTValue" (Student''s t-test p-value,'
'unpaired samples with unequal variance) and "pWValue" (Wilcoxon rank-sum test'
'p-value). The "qTValue" and "qWValue" represent the False-Discovery rate q-value'
'replacements for the "pTValue" and "pWValue" respectively. The q-values are'
'computed across all measures per "strain" (roughly 702 tests) and across "all"'
'strains and measures collectively (roughly 329 by 702 tests). The collection of'
...
'"name", a "unit" of measurement, titles for three possible subdivisions'
'("title1", "title2", and "title3" - the title of the feature itself, its motion'
'state, and its signed subdivision), helpful indexed offsets for these titles'
'("title1I", "title2I", and "title3I"), an associated struct "field" to locate'
'the feature in our other MAT files, the corresponding "index" for the struct'
'field (e.g., the six eigenworm projections are represented in a field, as a'
'6-element array), "isMain" (is this the main feature as opposed to a subdivision'
'of a main feature?), the feature "category" (morphology "m", posture "s", motion'
'"l", path "p"), the feature "type" (simple data "s", motion data "m", event'
'summary data "d", event data "e", inter-event data "i"), the feature "subtype"'
'(none "n", forward motion state "f", backward motion state "b", paused state'
'"p", event-time data "t", event-distance data "d", event-frequency data "h"),'
'and information regarding the feature''s "sign" (the feature is signed "s",'
'unsigned "u", is the absolute value of the data "a", contains only positive data'
'values "p", contains only negative data values "n").'
};
end
...
'Acknowledgements'
''
'WT2 employs Java (version 1.6) for the tracking software and Matlab (version'
'2010a) for the analysis software. Standard Matlab functions are used along with'
'the Image Processing, Statistics, and Bioinformatics toolboxes. Special thanks'
'and acknowledgements are due to Christopher J. Cronin and Paul W. Sternberg for'
'supplying the Matlab code from their publication "An automated system for'
'measuring parameters of nematode sinusoidal movement" (Cronin et al. 2005). Joan'
'Lasenby and Nick Kingsbury where an invaluable resource for computer vision'
'questions. Andrew Deonarine, Richard Samworth, Sreenivas Chavali, Guilhem'
'Chalancon, Sarah Teichmann, and Madan Babu provided a wealth of help and'
'information for the bioinformatic clustering and statistics. Finally, thanks and'
'acknowledgements are due for the following freely available code at the Matlab'
