function [worms imgs oImgs] = segWormFramesNative(videoFile, frames, ...
    verbose, varargin)
%SEGWORMFRAMES Segment the worm in a set of video frames and organize
%   the information in a structure.
%
%   [WORMS IMGS OIMGS] = SEGWORMFRAMESNATIVE(VIDEOFILE, FRAMES, VERBOSE)
%
%   [WORMS IMGS OIMGS] = SEGWORMFRAMESNATIVE(VIDEOFILE, FRAMES, VERBOSE,
%                                            SAMPLES)
%
...
vignetteFile = strrep(vignetteFile, '.mp4', '.info.xml.vignette.dat');
if ~strcmp(vignetteFile, videoFile) && exist(vignetteFile, 'file')
...
%         else
            ...
            %oImgs{i} = getframe(vr);
            timestamp = frames(i) * fps;
            img = vr.read(frames(i) + 1);
            oImgs{i} = img;
            ...
            % Segment the worm.
            worms{i} = segWorm(imgs{i}, frames(i), 1, verbose, varargin{:});
%         end
%     end
...
%             num2str(minutes, '%02.0f') ':' num2str(seconds, '%02.3f')]);
        disp(['Worm at frame = ' num2str(frames(i)) ...
            ', timestamp = ' num2str(hours) ':' ...
            num2str(minutes, '%02.0f') ':' num2str(seconds, '%02.3f')]);
        
        % Compute the proximity and head/tail movement confidence.
        if frames(i) < vr.NumberOfFrames - 1 % next(vr)
...
%                 
            ...
            %else
                nextOImg = vr.read(frames(i) + 2); %getframe(vr);
